...
class EclipseWellSmry(IWellSmry):
    ...
    def __init__(self, project_dir: Union[str, Path], project_name: str, lazy: bool = True):
        """
        初始化 Summary 数据。
        ...
        Args:
            project_dir: 项目目录路径
            project_name: 项目名称
            lazy: 是否懒加载。如果为 True，则只在第一次访问数据时才去解析巨型文件。
        """
        ...
        # 内部变量改为前置下划线，配合 @property 使用
        self._df_field: Optional[pd.DataFrame] = None
        self._df_region: Optional[pd.DataFrame] = None
        self._df_group: Optional[pd.DataFrame] = None
        self._df_well: Optional[pd.DataFrame] = None
    ...
    @property
    def df_field(self) -> Optional[pd.DataFrame]:
        if not self._parsed: self.parse_all()
        return self._df_field

    @property
    def df_region(self) -> Optional[pd.DataFrame]:
        if not self._parsed: self.parse_all()
        return self._df_region

    @property
    def df_group(self) -> Optional[pd.DataFrame]:
        if not self._parsed: self.parse_all()
        return self._df_group

    @property
    def df_well(self) -> Optional[pd.DataFrame]:
        if not self._parsed: self.parse_all()
        return self._df_well

    @property
    def df_conn(self) -> Optional[pd.DataFrame]:
        if not self._parsed: self.parse_all()
    ...
    def _parse_all_data(self) -> None:
        ...
        # 3. 构建各个域的数据立方体
        self._df_field = self._build_field_df(field_keys, df_time)
        self._df_well = self._build_entity_df(well_keys, df_time, 'well')
        self._df_region = self._build_entity_df(region_keys, df_time, 'region')
        self._df_group = self._build_entity_df(group_keys, df_time, 'group')
        self._df_conn = self._build_entity_df(conn_keys, df_time, 'conn_id')
    ...
    def get_well_data(self, well_name: str) -> pd.DataFrame:
        ...
        if self._df_well is None:
            return pd.DataFrame()
            
        return self._df_well[self._df_well['well'] == well_name].copy()
