...
class ZarrModelData(IModelData):
    ...
    def __init__(self, project_dir: str | Path, project_name: str):
        ...
        # 🌟 1. 从 static 组读取网格维度
        static_grp = self.root['static']
        self.nz, self.ny, self.nx = static_grp.attrs['frame_dim']
        self.meta['frame_dim'] = tuple(static_grp.attrs['frame_dim'])
        self.meta['frame_size'] = tuple(static_grp.attrs['frame_size'])
        self.meta['origin'] = tuple(static_grp.attrs['origin'])
        self.nblocks = static_grp.attrs['nblocks']
        self.radial = static_grp.attrs['radial']
        
        # 🌟 2. 从 dynamic 组读取各个步长的时间戳
        dyn_grp = self.root['dynamic']
        steps = []
        dts = {}
        for key in dyn_grp.keys():
            if key.startswith('Step_'):
                ...
                dt_str = dyn_grp[key].attrs.get('datetime')
                dts[step_idx] = datetime.fromisoformat(dt_str) if dt_str else None
                
        self.meta['dynamic_steps'] = sorted(steps)
        self.meta['dynamic_datetimes'] = dts
...
class ZarrWellData(IWellData):
    ...
    def __init__(self, project_dir: str | Path, project_name: str):
        ...
        comps_path = self.zarr_dir / "df_completions.parquet"
        if comps_path.exists():
            df_comps = pd.read_parquet(comps_path)
            for well, group in df_comps.groupby('well'):
                if well not in self._static_wells: self._static_wells[well] = {}
                self._static_wells[well]['completions'] = group[['I', 'J', 'K']].to_dict('records')

        self._trj_data = {}
        if 'trajectories' in self.root:
            trj_grp = self.root['trajectories']
            for well_name in trj_grp.keys():
                self._trj_data[well_name] = trj_grp[well_name][:]
            # 🌟 从轨迹组专属属性中取回 dates
            dates_str = trj_grp.attrs.get('dates', [])
            self.dates = [datetime.fromisoformat(d) for d in dates_str]
        else:
            self.dates = []
        
        events_path = self.zarr_dir / "df_events.parquet"
        self.df_events = pd.read_parquet(events_path) if events_path.exists() else pd.DataFrame()
...
class ZarrWellSmry(IWellSmry):
    """Zarr 产量动态替身 (原生无缝读取 Parquet 切片文件夹)"""
    def __init__(self, project_dir: str | Path, project_name: str):
        self.project_dir = Path(project_dir)
        self.project_name = project_name
        self.zarr_dir = self.project_dir / f"{self.project_name}.zarr"

        field_path = self.zarr_dir / "df_field.parquet"
        self._df_field = pd.read_parquet(field_path) if field_path.exists() else None
        
        well_chunks_dir = self.zarr_dir / "smry_well_chunks"
        if well_chunks_dir.exists():
            self._df_well = pd.read_parquet(well_chunks_dir)
        else:
            self._df_well = None
    ...
    # =========================================================
    # 🌟 补齐 IWellSmry 要求的抽象接口
    # =========================================================

    def get_well_data(self, well_name: str, property_names: Union[str, List[str]]) -> pd.DataFrame:
        if isinstance(property_names, str):
            property_names = [property_names]
            
        if self._df_well is None or self._df_well.empty:
            return pd.DataFrame()
            
        subset = self._df_well[self._df_well['well'] == well_name].copy()
        if subset.empty:
            return pd.DataFrame()
            
        base_cols = [c for c in ['step_idx', 'time_days', 'date'] if c in subset.columns]
        valid_props = [p for p in property_names if p in subset.columns]
        
        return subset[base_cols + valid_props]