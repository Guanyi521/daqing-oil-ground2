...
class TimeRoPE(nn.Module):
    ...
    def forward(self, x: torch.Tensor, datetimes: torch.Tensor, Ld: int):
        ...
        Args:
            ...
            datetimes (Tensor): 真实时间戳序列。形状: `(B, nt)`, dtype: int64 或 float32
        ...
        # 兼容推理阶段可能传入一维 (nt,) 的情况，自动补齐 Batch 维
        if datetimes.dim() == 1:
            datetimes = datetimes.unsqueeze(0)

        B, nt = datetimes.shape

        # 1. 转换为 float
        t = datetimes.to(device).float()  # (B, nt)

        # 2. 计算时间步对应的旋转角度
        # 使用广播机制代替 torch.outer，完美支持多维度
        # t.unsqueeze(-1) 形状: (B, nt, 1)
        # self.inv_freq 形状: (head_dim / 2)
        # 相乘后 freqs 形状: (B, nt, head_dim / 2)
        freqs = t.unsqueeze(-1) * self.inv_freq

        # 3. LLaMA 风格：前后两半维度拼接 -> (B, nt, head_dim)
        freqs = torch.cat((freqs, freqs), dim=-1)

        # 4. 🌟 时空广播魔法：按空间维度 Ld 展开
        # (B, nt, head_dim) -> (B, nt, 1, head_dim) -> (B, nt, Ld, head_dim) -> (B, nt * Ld, head_dim)
        freqs = freqs.unsqueeze(2).expand(-1, -1, Ld, -1).reshape(B, nt * Ld, -1)

        # 5. 适配输入注意力矩阵的维度 -> (B, 1, nt * Ld, head_dim)
        # 这样才能和 x: (B, num_heads, nt * Ld, head_dim) 进行广播相乘
