...
# 3. 引入您编写的数据集 (推荐使用绝对路径，以防在项目根目录运行引发 ModuleNotFoundError)
from fusion.dataset_unified_pipeline import DatasetUnifiedPipeline


def main():
    # 🌟 新增：开启 RTX 3090 的 Tensor Core 加速
    ...
    # 默认配置路径设为基于根目录运行的路径
    parser.add_argument('--config', type=str, default='fusion/unified_pipeline_config.yaml', help='YAML 配置文件路径')
    ...
    train_dataset = DatasetUnifiedPipeline(
        ...
        dynamic_feature_dim=config['model']['dynamic_feature_dim'],  # 🌟 修复：显式传入动态特征维度
        data_source_type = config['data'].get('data_source_type', 'zarr')  # 🌟 新增：显式传入读取类型
    )
    ...
    train_loader = DataLoader(
        ...
        prefetch_factor=config['training'].get('prefetch_factor', 2) if config['training'].get('num_workers',
                                                                                               0) > 0 else None,
        persistent_workers=True if config['training'].get('num_workers', 0) > 0 else False,
        # 👈 只有 num_workers > 0 才能设为 True
        pin_memory=True
    )
    ...
    if os.path.exists(poro_ckpt_path):
        ...
        print(f"  ⚠️ 警告: 未找到 Poro-VAE 权重 ({poro_ckpt_path})")
    ...
    if os.path.exists(perm_ckpt_path):
        ...
        print(f"  ⚠️ 警告: 未找到 Perm-VAE 权重 ({perm_ckpt_path})")
    ...
    model = LatentDynamicsPipeline(
        ...
        poro_dim=config['geo_vae_poro_config']['latent_channels'],
        perm_dim=config['geo_vae_perm_config']['latent_channels'],
        dynamic_feature_dim=config['model']['dynamic_feature_dim'],  # 🌟 补上这个缺失的传参
    ...
    trainer = pl.Trainer(
        ...
        devices=1,  # 🌟 强制只使用 1 张显卡
        strategy="auto",  # 🌟 取消 DDP，使用自动策略
        logger=logger,
