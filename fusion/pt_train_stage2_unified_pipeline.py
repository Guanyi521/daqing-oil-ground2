...
def main():
    ...
    train_loader = DataLoader(
        ...
        num_workers=config['training'].get('num_workers', 8),  # 接收 yaml 中的大进程数
        prefetch_factor=config['training'].get('prefetch_factor', 4),  # 🌟 接收预取参数，疯狂消耗内存囤积数据
        persistent_workers=True,  # 🌟 关键：让这些高内存占用的工人常驻后台，提升连续性
        pin_memory=True
    )
