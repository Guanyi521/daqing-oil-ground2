...
# import os
# os.environ["TMPDIR"] = "F:/temp_cache"
# os.environ["TEMP"] = "F:/temp_cache"
# os.environ["TMP"] = "F:/temp_cache"
# # 确保文件夹存在
# if not os.path.exists("F:/temp_cache"): os.makedirs("F:/temp_cache")
        ...
        for p_dir, p_name in chunk_projects:
            try:
                ...
                sampler = EclipseDataSampler(
                    ...
                    data_source_type=self.data_source_type,  # <--- 关键透传
                ...
                for w_name, seq in well_buffer.items():
                    ...
                    for start in range(0, history_len - self.nt_width, self.stride):
                        ...
                        # 1. window_fields: 动态场数据，必须转为 float32
                        window_fields = torch.stack([s['field_data'] for s in window]).float()

                        # 2. datetimes: 已经指定了 float32，保持原样即可
                        datetimes = torch.tensor([s['step_idx'] for s in window[:-1]], dtype=torch.float32)

                        # 3 & 4. 获取对应的静态场：孔隙度和渗透率也必须转为 float32
                        poro_field = poro_dict[w_name]['field_data'].float()
                        perm_field = perm_dict[w_name]['field_data'].float()

                        # 5. dynamic_features: 井控特征，同样需要转为 float32
                        dynamic_features = torch.stack([s['dynamic_control'] for s in window]).float()

                        # 6. spatial_mask_orig: 静态掩码
                        # 🌟 极其重要！因为 VAE 内部会执行 h * mask，如果 mask 是 double 会直接导致崩溃
                        spatial_mask_orig = poro_dict[w_name]['mask'].float()

                        # 7. hard_data_locs: 坐标索引通常是整数(long)，不需要加 .float()
...
if __name__ == "__main__":
    ...
    # 填入你自己的测试路径
    PROJECT_DIR = r"F:\jbgs\jbgs-model-training\dataset\water_196311-199307"
